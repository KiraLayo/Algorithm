...
class Solution {
    func calculateTime(_ keyboard: String, _ word: String) -> Int {
        var keyboardIndex:[Character:Int] = [Character:Int]();
        
        keyboard.enumerated().forEach { (char) in
            keyboardIndex[char.element] = char.offset;
        }
        ...
        for char in word {
            res += abs(keyboardIndex[char]! - pre);
            pre = keyboardIndex[char]!;
        }
